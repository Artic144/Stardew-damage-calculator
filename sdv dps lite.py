...
###############################|| USER INPUTS ||###################################
# These are used for testing or debugging. Should not be active when run to generate .csvs
...
# "Crusader", "Bug Killer", "Slime Slayer" are valid
input_weapon_enchants = []


###############################|| ITERATING INPUTS ||###################################

def generate_inputs(weapon,input_weapon_enchants):
    ...
    elif input_weapon_enchants.count("Slime Slayer") > 0:
...
def calculate_damage(weapon, forges, innate_enchant, rings, professions, extra_luck, extra_atk, weapon_enchants):
    ...
    # should not be active when generating .csvs
    '''
